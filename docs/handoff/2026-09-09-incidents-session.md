...
Усе нижче **виміряне**, не припущене. Питання `❓` нижче **зняті** 09.09: кейс повільності закрито користувачем — причина Windows-апдейти (§8).
...
| Нитка | Стан | Коротко |
|---|---|---|
| Повільна локалка 08–10 (Google, Bitwarden) | ✅ **закрито** | **сатурація UT-лінка хвилею Windows-апдейтів (Patch Tuesday +1)** — див. §8; плюс окремо виправлений тихий QUIC-дроп на PA (постійний податок на Chrome) |
| Bitwarden тупить у вкладці | ✅ закрито | та ж сатурація каналу: тест був у вікні 08–10, аплінк спільний для дроту й Wi-Fi — тому «та ж картина». Не ноут |
...
| Wi-Fi ноута флапає | 🟠 не відтворилось | 09.09: 21 хв пінгів по 1 с — 0 розривів; невідомо, дріт чи Wi-Fi був активний; фікс «Intel Roaming Aggressiveness» до ChromeOS не застосовний |
| conveniq beacon | ✅ закрито 09.09 | не відновився після TTL: остання сесія 19.08 11:41; RustDesk перецілено на наш `gw2.modern-expo.com` (168.119.209.246); див. `docs/changes/2026-09-09-conveniq-beacon-zakryto.md` |
...
> ✅ **Питання 1 — закрито виміром 09.09** з самого ноута (Chromebook, контейнер Crostini): QUIC Initial до Google/Fastly/Facebook/Cloudflare → `EHOSTUNREACH` за 0.7–1.3 мс, TCP-фолбек миттєвий. Нюанс методу: незʼєднаний UDP-сокет у Linux не бачить ICMP-помилок — перший прогін показав хибний «тихий дроп»; міряти лише `connect()`-сокетом або `IP_RECVERR`.

### 1.2 Bitwarden: ЗНЯТО 09.09 — причина та ж сатурація каналу (§8), не ноут. Нижче — хід розбору як був
...
| Що | Стан | Що потрібно |
|---|---|---|
| **conveniq.net beacon** (Conference-box-1, 172.18.53.78) | ✅ закрито 09.09 | після спливання TTL не відновився (0 сесій до LB за 21 день після 19.08 11:41); коробка жива, RustDesk ходить на корпоративний `gw2.modern-expo.com`. Трафік парку до LB — ICMP ping + TCP-конекти без даних (моніторинг), не HTTP |
...
**ЗАКРИТО 09.09 ~13:15 користувачем: «кейс по тормозам закриваємо — то вінда апдейти».** Це ж закриває і Bitwarden (§1.2):
...
## 10. ДОПОВНЕННЯ 11.09 — POCO-тунель закрито (mitigated), зміна PA зайшла на всю LAN

- **1352116** (той самий POCO-M5 → 5.254.60.210:53, 11 МБ/131 МБ) + **1346180** закриті Non-Issue/mitigated.
  За 7 діб телефон ходив на 6 адрес Voxility по tcp/53; внутрішніх ресурсів не чіпав.
- **Власник:** з логів не дістати (PA без User-ID на VLAN 104, NPS MAC не бачив, WLC не логує). За експортом
  Endpoint Management (`~/devices.csv`, коди моделей: POCO M5 = 22071219CG, M5s = 2207117BPG) єдиний
  POCO M5 — **Andriy Laganovskyy**, робочий профіль без синхронізації з 19.05.2024 (системна прогалина:
  me_mobile пускає пристрої зі «сплячим» профілем; me_mobile = особисті телефони з робочим профілем, не гості).
- **Мітигація власника:** (1) `unknown-tcp`/`unknown-udp` прибрані з app-group `MEG Internet Applications`
  → «Allow Base Applications LAN - WAN» їх не матчить → `Default Deny LAN` (тихий deny). **Ефект LAN-wide:**
  за 10 хв 260 denies unknown-tcp:443 із VLAN 96/100/108 до Telegram DC (149.154.166.x) і TeamViewer —
  власник підтвердив, що Telegram дозволений окремою app-group для потрібних хостів, це задумана політика.
  (2) Ключ SSID me_mobile зротовано 11.09 ~14:00; телефон офлайн з 14:06:38, повторного DHCP немає.
- **Стежити:** поява `POCO-M5` у DHCP-логах після ротації = власник прийшов за ключем.
- **WLC:** керування 172.17.65.4 з VLAN 61 недосяжне (TCP відкривається, SSH-банер/HTTPS не приходять,
  ACL = VLAN 605), 172.17.61.10 закриває SSH. MAC-виняток і так марний: рандомізований per-SSID MAC
  міняється при повторному приєднанні, яке ротація ключа і спричиняє. Блокувати на PA за app/port.
- **Черга на кінець дня:** 1 новий — 1355666 «Process Launched from Temp Directory» з 192.168.50.67
  (mag 5, AI 0.7, tier-2 впав) — у розборі.
