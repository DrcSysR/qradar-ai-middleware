...
| `score ≤ refset_cleanup_max_score` (0.3), не mitigated | закрити **і зняти IP із блок-листа**, якщо для правила заданий refset і entity=IP |
...
- **Право знімати блок усе ще має tier-1 (відкрито з 2026-09-03).** Поріг розблокування
  відокремлено від порогу закриття (`refset_cleanup_max_score`, дефолт 0.3) після того, як
  пайплайн зняв блок з активних SSH-сканерів 63 рази за добу по 29 адресах на вердикті
  `Suspicious_Keep_Watching` 0.4 — запис 2026-09-03. Але 0.3 усе ще досяжний для tier-1
  (llm01, 3B), тобто найслабша модель формально може розблокувати файрвол без перегляду:
  `escalate_threshold` 0.6 такі вердикти на tier-2 не піднімає. Структурний крок —
  вимагати tier-2 для будь-якого розблокування — не зроблений.
