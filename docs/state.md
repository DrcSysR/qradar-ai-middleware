...
- **Поллер насичений постійно (переміряно 2026-08-27).** Ліміт 100 офенсів/ран, вікно
  ...
  Для реального дренажу треба ~8× потужності: `poller_concurrency` у `config.json` **не
  заданий** (дефолт 3) і впирається в 3 воркери gunicorn — піднімати їх треба разом.
