...
**Актуально на:** 2026-09-23 · **Керує:** скіл `qradar-soc` + `CLAUDE.md` репо · **Прод:** так
...
```
QRadar offenses API ──► poller.py (кожні 10 хв, вікно 48 год, БЕЗ ліміту — продюсер)
...
| `score ≤ 0.6`, не mitigated | закрити офенс; блок ЛИШАЄТЬСЯ |
| `score ≤ refset_cleanup_max_score` (0.3), не mitigated, **вердикт від tier-2** | закрити **і зняти IP із блок-листа**, якщо для правила заданий refset і entity=IP |
| той самий випадок, але tier-2 не переглядав | закрити, блок ЛИШАЄТЬСЯ (fail-safe) |
...
- **Пропускна здатність після ресету (2026-08-25).** Обсяг 1534 → 4838 офенсів/добу.
  ...
  завжди (раніше 92% закривались без неї). Лікувалось паралельним поллером
  (`poller_concurrency`, дефолт 3) — запис 2026-08-25-b; з 2026-09-23 ключ прибрано,
  конкурентність переїжджає у `worker_concurrency` (крок B).

- **Поллер більше не вибирає, що обробляти (2026-09-23).** Квота рану і ліміт 100/ран
  прибрані: поллер став продюсером черги `work_queue`, порядок обробки задає магнітуда
  (запис 2026-09-23). Причина — 150 зі 158 відкритих mag-7 injection-офенсів жодного
  разу не дійшли до мідлваря, конкуруючи за 100 місць із low-mag шумом. Абзац нижче —
  історія того, як черга вирівнювалась *до* цього.
...
- **Розблокування: окремий поріг + обов'язковий tier-2 (2026-09-03).** До 03.09 IP
  знімався з блок-листа за тією ж умовою `score <= 0.6`, що й авто-закриття — і пайплайн
  розблоковував активні SSH-сканери 63 рази/добу по 29 адресах на вердикті
  `Suspicious_Keep_Watching` 0.4 (запис 2026-09-03). Тепер зняття блоку вимагає
  `score <= refset_cleanup_max_score` (0.3) **І** вердикту важкої моделі
  (`refset_cleanup_requires_tier2`, дефолт true) — кандидат на розблокування сам тригерить
  ескалацію попри низький score. Немає перегляду (каскад вимкнений / ескалація впала) →
  блок лишається з явною нотаткою. Побічний ефект, який треба тримати в голові: при
  `escalate_enabled: false` розблокування не працює взагалі.
...
- **Покриття vs тріаж (переміряно 2026-09-03, після догінних проходів).** Борг пайплайну
  вичерпано, лишилось покриття. Відкритих офенсів 1852 (2053 на 02.09); бакет «матчиться,
  але поза вікном поллера» **852 → 15**, застряглих `AQL_ERROR` **166 → 49**. Механізм —
  `tools/catchup.py` (записи 2026-09-02 і -b). `NO_EVENTS` (114 → 133) свідомо не чіпали:
  там AQL порожній після бенін-фільтрів, повтор дасть те саме.
  **Єдина реальна діра тепер — 595 відкритих офенсів без ключа в `prompts.json`**:
  `IRC Connections`, `Login failure to an expired account`, `Suspicious Web Server
  Activities`, `Detected a Service Binary Path Changed`, `Suspicious Number of Same User
  Logins to Multiple Devices`, `Abnormal Parent for a System Process`. Це вже не борг
  механізму, а робота з написання юзкейсів.
...
- **Черга аналітика (переміряно 2026-09-02).** Призначених відкритих офенсів — 29
  (14 на 27.08, 0 на 26.08). Переоцінка всіх 29 через поточні промпти показала, що **2/3
  черги — це вже виправлені FP**: 13 упали з 0.8–0.95 до ≤0.4, ще 2 — у 0.5–0.6.
  Причина, чому вони висіли: після правки промпта чи AQL старий офенс має статус
  `PROCESSED` і його не бере **ні поллер, ні догінний прохід** — тобто кожна правка
  лікує лише майбутні офенси, а ті, що її спричинили, лишаються в черзі із застарілим
  вердиктом. Закрито прапорцем `force` у тілі запиту (обхід «вже оброблено» з
  увімкненими авто-діями) — запис 2026-09-02-b, там же розбір чотирьох гарячих.
