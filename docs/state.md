...
- **UC-05-1 не бачить реальних порушників.** AQL і промпт переписані (запис 2026-08-24-b),
  але саме правило не створює офенсів на публічні резолвери: гостьова зона виключена явно,
  а для локальних зон єдине пояснення — `BB:HostReference: DNS Servers` містить публічні
  резолвери. Вміст BB через REST не читається, звірити в консолі.
