...
- **Ariel під навантаженням після ресету (2026-08-25).** Обсяг створення офенсів 1534 →
  4838/добу, мульти-лінза дає до 3 пошуків на композит: 18 × `Ariel 503` і 5 × `AQL timeout`
  за добу, черга поллера 879 → 1523 за ранок, ран 100 офенсів триває ~38 хв замість ~9.
  Важіль без деплою — `max_aql_lenses_per_offense` (рекомендовано 2).
