...
**FIRST CHECK — "Into Other Hosts" requires a REAL lateral target. Do this before scoring.**
The offense name "Detected a Successful Login From a Compromised Host Into Other Hosts" promises lateral movement. Verify it actually happened. List the DISTINCT `destinationip` of the logon / "Successful Login From a Compromised Host" / "UserID login" events, then:
  1. `sourceip == destinationip` (self-logon, ANY Logon Type incl. LT=5 service / LT=3 network, often `SYSTEM` / `palo.alto`) → NOT lateral movement (pattern E).
  2. `destinationip == 172.17.64.101` with a COMPUTER account (`<NAME>$`, e.g. `MNG169$`, `MNG68$`, `MNG169$@MODERN.ORG`) or a service user → this is the **Palo Alto User-ID agent** pushing IP↔user mappings to the firewall, NOT a login into a host (pattern J). Frequently the single highest-count event in these offenses (often >1000) — discount entirely.
  3. `destinationip` is a Domain Controller for routine Kerberos/LDAP → NOT lateral movement.
  4. `destinationip` is in the shared RDS/app farm `172.17.61.0/24` (V-SRV10–V-SRV13 = `.189/.124/.126/.150`, also `.42/.44`) reached by a regular `firstname.lastname` user with their OWN creds via LT=3/10 → routine shared-server usage (pattern I). Multiple unrelated users hitting the same farm set is the tell.
IF every destination falls into 1–4 above → there is NO login "into other hosts": output `score` 0.0–0.2, `verdict` "FP cascade — …", and recommend dropping the host from the `Compromised Hosts` refset. Do NOT inflate the score for X-Force volume to `94.153.123.0/24` (pattern F — stale Ukrtelecom feed) nor for the raw count of self / User-ID "repeated successful logins from the same IP". Only escalate above 0.6 when at least one destination is a SENSITIVE host OUTSIDE the 61.x farm reached with admin/service creds, OR genuine PowerShell tradecraft is present (see Real Red Flags).
Allowed verdict strings when this check trips: "FP cascade — refset loop", "FP cascade — self-logon loop", "FP cascade — User-ID agent", "FP cascade — routine RDS usage", "FP cascade — stale X-Force".
...
C) Sub-rule "Thread Creation into a System Process" firing on Sysmon EventID=8 where:
   ...
   - `Image`/`SourceImage` AND `TargetImage` are BOTH signed binaries under the SAME vendor `Program Files` folder, with `StartFunction: RtlpQueryProcessDebugInformationRemote` (StartModule `ntdll.dll`) — this is an app's own updater/licensing process querying debug info across its own processes. Benign. Observed: Autodesk AdODIS (`C:\Program Files\Autodesk\AdODIS\V1\Setup\AdskAccessServiceHost.exe` → `AdskExecutorProxy.exe` / `AdskUpdateCheck.exe`), `SourceUser == TargetUser`.
...
D2) "Powershell Has Been Launched in a Compromised Host" sub-rule firing on a Process Create where `ParentImage: C:\Windows\System32\CompatTelRunner.exe` and `Image: …\powershell.exe -ExecutionPolicy Restricted -Command …` run as `SYSTEM` (command bodies like `Write-Host 'Final result: 1'`, `$Res = 0`, `function Get-UEFIX509Certificate…`). This is the Windows Compatibility Appraiser / telemetry doing UEFI Secure-Boot certificate checks. `-ExecutionPolicy Restricted` (not Bypass), no `-EncodedCommand`/IEX/DownloadString. Benign OS telemetry.
...
I) Sub-rule "Successful Login From a Compromised Host (Into Other Hosts)" / lateral-movement firing on a REGULAR named user (firstname.lastname) logging in with their OWN credentials via RDP (`Logon Type: 10`) or Network (`Logon Type: 3`) FROM a workstation (172.17.97-99.x range) TO the internal Remote-Desktop / application server farm. In this environment the `V-SRV*` hosts in `172.17.61.0/24` — e.g. `172.17.61.124` (V-SRV11), `172.17.61.126` (V-SRV12), `172.17.61.150` (V-SRV13), `172.17.61.189` (V-SRV10), plus `172.17.61.42` and `172.17.61.44` — are SHARED RDS/app servers that employees use daily. Treat the whole `172.17.61.0/24` server subnet as candidate shared-farm: multiple unrelated `firstname.lastname` users with OWN creds converging on the same small dest set = normal usage. Multiple unrelated users (e.g. different firstname.lastname) RDPing to the same V-SRV set is the signature of normal shared-server usage, NOT attacker lateral movement. Treat as cascade noise driven by the host's `Compromised Host` refset membership. This is the dominant FP for the "Powershell Has Been Launched in a Compromised Host" cascade — when the only "red flag" is RDP/Network logon to this farm by a regular user with their own creds and there is no genuine PowerShell tradecraft, score it 0.0–0.3 and recommend dropping the host from the refset.

J) "UserID login" events (LogSourceType `Palo Alto PA Series`) where the destination is `172.17.64.101` (the Palo Alto User-ID collector/firewall) and the username is a COMPUTER account (`<NAME>$`, e.g. `MNG169$`, `MNG68$`, `MNG169$@MODERN.ORG`) or a service account — this is the Palo Alto User-ID agent pushing IP↔user mappings to the firewall, NOT a user logging into another host. These are routinely the single highest-volume event in this offense (often >1000 in one offense). They are NOT "logins into other hosts" and must NOT be counted as lateral movement or as "repeated successful logins from the same IP". Benign.
...
- 0.0–0.3 CLEAR FP CASCADE: events are dominated by FP patterns (A, B, C, D, D2, E, G, H, J) and/or the only "lateral movement" is routine RDP/Network logon by a regular user with their own creds to the V-SRV* RDS/app farm (pattern I), self-logon (E), or Palo Alto User-ID agent pushes to 172.17.64.101 (J). High raw counts of self / User-ID "repeated logins" and X-Force volume to 94.153.123.0/24 (F) do NOT raise the score. No process-create or thread-injection events with hostile shape, no PowerShell tradecraft. Recommend dropping host from `Compromised Hosts` refset.
